...
export default function BankBook() {
  ...
  const load = async () => {
    ...
    setLoading(true)
    try {
      const { data: res } = await api.get('/reports/bank-book', { params: { company: company._id, ledger: selectedLedger._id, from, to } })
      setData(res)
  ...
  return (
    <div style={{ padding: 16, height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, borderBottom: '1px solid #243358', paddingBottom: 8 }}>
        <div style={{ color: '#FCAF1E', fontSize: 13, fontWeight: 700 }}>BANK BOOK — {company?.name}</div>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <div style={{ width: 200 }}>
            <LedgerSearch value={selectedLedger} onChange={setSelectedLedger} placeholder="Select Bank Ledger..." groupNature="assets" />
          </div>
      ...
      {!selectedLedger ? (
        <div style={{ color: '#8B9DC3', textAlign: 'center', marginTop: 40 }}>Select a bank ledger to view the bank book</div>
      ) : loading ? <div style={{ color: '#8B9DC3' }}>Loading...</div> : !data ? null : (
        <>
          <div style={{ marginBottom: 8, display: 'flex', gap: 20 }}>
            <span style={{ color: '#8B9DC3' }}>Opening: <span style={{ color: '#34D399', fontWeight: 600 }}>₹{fmt(Math.abs(data.openingBalance))}</span></span>
            <span style={{ color: '#8B9DC3' }}>Closing: <span style={{ color: data.closingBalance >= 0 ? '#34D399' : '#F87171', fontWeight: 600 }}>₹{fmt(Math.abs(data.closingBalance))}</span></span>
          </div>
            ...
            <table className="tally-table">
              <thead>
                <tr>
                  ...
                  <th className="tally-th" style={{ textAlign: 'right' }}>Receipts (Dr)</th>
                  <th className="tally-th" style={{ textAlign: 'right' }}>Payments (Cr)</th>
